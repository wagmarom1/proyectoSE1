    ...
    ui(new Ui::MainWindow)
{
    ...
    system("amixer sset 'HeadsetR Mixer AudioR1' on");
    ...
    ui->BackGndLabel->setText(ImagePath + "bg.jpeg'/><br>");
}
    ...
    else if(selectedIndex == 0) //Local
    {
        ...
        else
        {
            ...
            if(songPath != "")
            {
                ...
                _mp3Admin->playLocal(songPath);  // Normal Local playing
            }
        }
    }
    else //remoto
    {
        _mp3Admin->stopRemoteSong(selectedIndex - 1, songPath);
    }
}
...
{    
    ...
    if( songPath != "" &&  ip!= "" && portNumber != 0  && nameRemote != "")
    {
        ...
        _mp3Admin->playRemote(-1, songPath, ip, portNumber); //UDP remote playing
        //_mp3Admin->playStopSong(true, songPath, ip, portNumber);
    }
}
...
void MainWindow::on_volumeSlider_sliderPressed()
{
    ...
    QString cmd = "amixer sset 'DAC1 Digital Fine' " + QString::number(value);
    QByteArray bb = cmd.toLatin1();
    const char *word = bb.data();
    g_print("Command: %s \n", word);
    system(word);
}
